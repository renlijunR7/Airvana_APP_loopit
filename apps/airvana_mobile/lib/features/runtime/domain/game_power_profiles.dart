/// arcade 作品的能力需求声明，每个 `assets/arcade/<slug>/` 一条。
///
...
class GamePowerProfile {
  ...
  /// 这个作品**自身代码实际会请求**的系统权限。
  ///
  /// 它不是从 [requirements] 推导的——「声明需要二维指针」不等于「会去开
  /// 摄像头」，按控制语义推权限会把摄像头授权面发给每一个有指针操作的作品。
  /// 取值来自逐个作品 grep `getUserMedia` 与 `navigator.geolocation`：
  /// 只有两个手势作品会请求，而且都只要 video；其余作品一律空集。
  ///
  /// 宿主用它收窄权限代答面：没声明的权限，作品请求了也一律拒绝。
  final Set<PowerPermission> permissions;

  /// 是否受 `assets/arcade/manifest.json` 的「不改动」承诺约束。
  ///
...
const List<GamePowerProfile> kGamePowerProfiles = <GamePowerProfile>[
  GamePowerProfile(
    ...
    // app.js 调 getUserMedia({video:{facingMode:'user'}})，跑本机 MediaPipe。
    permissions: {PowerPermission.camera},
    genre:
        '4 步定制向导 + 摄像头手势驱动的 3D 粒子圣诞树演示（贺卡/氛围类互动，无关卡、无分数、无胜负；7 个手势各自映射一种粒子效果：聚拢/爆散/旋转/缩放/星光/文案切换）',
    sealed: false,
    ...
    requirements: [
      ...
      PowerRequirement(
        ...
        dimensions: 1,
        minHz: 30,
        maxLatency: Duration(milliseconds: 80),
        optional: true,
      ),
    ],
  ),
  GamePowerProfile(
    slug: 'city-squad',
    genre:
        '3D 第三人称「算术门跑酷 + 队伍集结」射击：横向滑动控制小队在公路上左右走位，穿过 ×/÷/+/− 算术门增减人数，躲避电锯/铁锤障碍，自动向前开火清怪，限时内击败 Boss',
    sealed: true,
    ...
    requirements: [
      ...
      PowerRequirement(
        ...
        dimensions: 1,
        minHz: 30,
        maxLatency: Duration(milliseconds: 120),
        optional: false,
      ),
      ...
      PowerRequirement(
        ...
        dimensions: 1,
        minHz: 10,
        maxLatency: Duration(milliseconds: 250),
        optional: true,
      ),
  ...
  GamePowerProfile(
    slug: 'coin-castle',
    genre:
        '3D 推金币（coin pusher）休闲街机：在台面横向选落点投币，靠往复推板把金币/宝石/礼盒/宝箱从前沿推落计分；每投 10 枚可放一次「强力推」。three.js 渲染，单',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
      ...
      PowerRequirement(
        ...
        dimensions: 1,
        minHz: 8,
        maxLatency: Duration(milliseconds: 150),
        optional: false,
      ),
  ...
  GamePowerProfile(
    slug: 'coin-dozer',
    genre:
        '2.5D Canvas 推币机(coin pusher)：横向选一个落点把币投下去，往复推板把台面币推向前沿落袋，附带宝石收集、连击、三种道具(挡板/摇一摇/巨型币)与 6 套玩法',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
      ...
      PowerRequirement(
        ...
        dimensions: 2,
        minHz: 60,
        maxLatency: Duration(milliseconds: 60),
        optional: true,
      ),
  ...
  GamePowerProfile(
    ...
    requirements: [
      ...
      PowerRequirement(
        ...
        dimensions: 1,
        minHz: 5,
        maxLatency: Duration(milliseconds: 400),
        optional: true,
      ),
    ],
  ),
  GamePowerProfile(
    slug: 'hide-online',
    genre: '第一人称办公室捉迷藏（Three.js 实时 3D，躲藏者变身物品 / 猎人搜捕）',
    sealed: false,
    injectable: GameInjectable.full,
    requirements: [
      // 虚拟摇杆移动：连续二维模拟量，是核心操作。
      PowerRequirement(
        control: PowerControl.pointer,
        dimensions: 2,
        minHz: 30,
  ...
  GamePowerProfile(
    slug: 'pelican-bike',
    genre: '海岸线骑行巡航（Three.js 实时 3D，昼夜与天气全程程序化生成）',
    sealed: false,
    injectable: GameInjectable.full,
    requirements: [
      // 左右转向：离散按住式，不是连续摇杆。
      ...
      // 可选的倾斜转向：俯视巡航玩法，倾斜能自然映射到方向，但作品只接了按钮。
      PowerRequirement(
        control: PowerControl.lane,
        dimensions: 1,
        minHz: 30,
        maxLatency: Duration(milliseconds: 100),
        optional: true,
      ),
    ],
  ),
  GamePowerProfile(
    slug: 'htx-quest',
    genre:
        '横版自动奔跑跑酷（auto-runner）：角色自动向右跑，玩家只负责「跳跃（可二段跳）」和「按住下滑」两个动作躲障碍/断崖并收集 Token 碎片；死亡后进入三选一知识问答换复活',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'kol-town',
    genre:
        '非实时的 2.5D 等距小镇经营/叙事枢纽：拖动地图 → 点建筑走位进屋 → 在弹窗里玩 6 个轻量子玩法（3 题问答、8 秒二选一方向预测、直播点亮×3、三幕剧情、3/4 枚收集',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'lucky-fruit',
    genre:
        '24 格环形跑灯式水果老虎机：先在 8 个水果上离散押注，按「开始」后跑灯定格开奖，中奖可再猜一次大小翻倍——完全回合制，没有任何连续操作或实时操作窗口。',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'mini-gp-racers',
    genre:
        '3D 第三人称方程式竞速：单圈限时挑战，目标是在倒计时内完成指定超车数，赛后结算赛车币并回车库买车/升级（不是打包产物，是可读源码：index.html + core.js/dri',
    sealed: true,
    injectable: GameInjectable.partial,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'niguolaia',
    genre:
        '波次制塔防：在 6 个固定空地建塔/升级/拆除，外加两个需要在战场上自选落点的 AoE 技能（闪电风暴、招兵），整体是慢节奏的布防决策，不是即时操作。',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'poker-training',
    genre: '德州扑克新手训练桌（Three.js 实时 3D + GLB 角色，固定教学局与自由练习）',
    sealed: false,
    injectable: GameInjectable.full,
    requirements: [
      // 下注/跟注/弃牌都是离散按钮，这是核心操作。
      PowerRequirement(
        control: PowerControl.trigger,
        dimensions: 1,
        minHz: 5,
  ...
  GamePowerProfile(
    slug: 'risk-run',
    genre:
        '单列下落物收集（篮子横向接 / 直点收集两种模式）+ 18 套「空投版本」规则面板按钮决策，外层包一个 KOL 活动运营壳（AI 聊天 + Live2D 形象 + 任务/钱包/兑换',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'star-table',
    genre:
        '回合制"观战竞猜"UI：四阶段状态机(预约→实时竞猜→观战→结算)，玩家只做三件事——从 6 名 AI 选手里挑 1 个、从 100/200/400 三档里挑 1 个星币、按确认；',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    ...
    genre:
        '三跑道自动跑酷（Talking Tom Gold Run 型）：角色恒定前进，玩家只负责左右换道 / 跳跃 / 滑铲躲避障碍并吃金条，周期性进入浣熊首领 Boss 战，需要跳起把炸',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'sud-texas',
    genre:
        '回合制 9 席德州扑克单机复刻：玩家对本地 AI 机器人，每回合在「弃牌 / 让牌·跟注 / 加注·下注 / 全下」四选一，加注额用一条竖向滑杆定量；全程无实时循环，只有 15 秒',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
  ...
  GamePowerProfile(
    slug: 'token-harbor',
    genre:
        '3D 等距「放置经营」：拖动虚拟摇杆操控小人在港口跑动，靠近空投堆自动采集代币，背回柜台卖给 NPC 顾客换 USDT，再买升级和雇员扩张 BTC/ETH/SOL/BNB 四条产线',
    sealed: true,
    injectable: GameInjectable.full,
    requirements: [
...
/// 按 slug 取作品档案；未收录的作品返回 null。
GamePowerProfile? gamePowerProfileFor(String slug) {
  for (final profile in kGamePowerProfiles) {
    if (profile.slug == slug) return profile;
  }
  return null;
}

/// `assets/runner` 下那套内置作品实际会请求的权限。
///
/// `sensor-interactions-v1.js` 的 PROFILES 只有三种 kind：orientation、
/// motion、microphone。前两者在 Android 上不经 `onPermissionRequest`
/// （DeviceOrientationEvent 不需要运行时授权），所以真正需要代答的只有麦克风。
/// 没有任何一个内置作品调摄像头。
const Set<PowerPermission> kRunnerGamePermissions = {
