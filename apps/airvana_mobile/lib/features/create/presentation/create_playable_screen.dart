...
import 'power_presentation.dart';
...
class _CreatePlayableScreenState extends State<CreatePlayableScreen> {
  ...
  /// （`public/index.html` 的 catalog 与 `creationCapabilityIds` 白名单）。
  /// 四份副本已经漂移——Web 两份都少了 `failureRetry`，而
  /// `sanitizeCapabilityIds` 会把不在白名单里的 id **静默丢弃**，
  /// 于是「新增一个能力」变成「改四个地方，漏一处就无声失效」。
  static final List<_PowerDefinition> _powerCatalog = kPowerCatalog
      .map(
        (power) => _PowerDefinition(
          power.id,
          power.category,
          power.title,
          power.description,
          kPowerIcons[power.id] ?? Icons.bolt_rounded,
          support: _PowerSupport.values.byName(power.support.name),
          dependencies: power.dependencies,
          conflicts: power.conflicts,
          quickAllowed: power.quickAllowed,
          visibleInCatalog: power.visibleInCatalog,
        ),
      )
