...
(function(root){
  ...
  function terrain(c,x,y,w,h){const entry=resource('spring-dig');if(entry.status!=='ready')return false;const img=entry.image;c.drawImage(img,x/360*img.naturalWidth,y/560*img.naturalHeight,w/360*img.naturalWidth,h/560*img.naturalHeight,x,y,w,h);return true;}
  ...
  root.AirvanaPhysicsArt=Object.freeze({version:'2.0.0',keys:()=>keys.slice(),background,sprite,preload,terrain,poster,status:()=>Object.fromEntries([...cache].map(([key,v])=>[key,v.status]))});
