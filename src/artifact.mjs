...
export function buildArtifact({ content, payload, version }) {
  ...
  const manifest = {
    ...
    // 本成品可消费投放期注入的分桶（globalThis.__AIRVANA_VARIANT__）；旧成品无此能力，投放层据此判断而不是假定
    variantAware: true, optimizableFields: ['title', 'hook', 'coverStyle', 'interactionOrder', 'difficulty'],
  };
  ...
  const DATA=${data};const VARIANT=(globalThis.__AIRVANA_VARIANT__&&typeof globalThis.__AIRVANA_VARIANT__==='object')?globalThis.__AIRVANA_VARIANT__:null;
  let sessionToken=null;let sequence=1;let checkpointSent=false;let requiredChecks=2;const $=s=>document.querySelector(s);
  ...
  applyVariant();
  function bridge(type,payload={}){try{if(globalThis.AirvanaBridge&&typeof globalThis.AirvanaBridge.postMessage==='function')globalThis.AirvanaBridge.postMessage(JSON.stringify({version:1,type,payload}))}catch{}}
  ...
  async function event(type,payload={}){const sentSequence=sequence;const result=await api('/api/runtime/events',{sessionToken,sequence,eventType:type,payload});sequence+=1;bridge('runtime_event_accepted',{eventType:type,sequence:sentSequence,accepted:result.accepted===true,points:Number(result.points||0),rewardStatus:String(result.rewardStatus||'not_applicable')});return result}
  ...
  function renderGame(){const steps=(DATA.payload.sections||[]).slice(0,requiredChecks);let i=0;const draw=()=>{const s=steps[i]||{heading:'最终确认',body:'完成最后一个行动检查即可结束体验'};$('#step-title').textContent=s.heading;$('#step-body').textContent=s.body;$('#choices').innerHTML='<button id="choice">完成这个行动检查（'+(i+1)+'/'+requiredChecks+'）</button>';$('#choice').onclick=async()=>{try{if(!checkpointSent){await event('step_complete',{step:i+1});checkpointSent=true}i+=1;if(i>=requiredChecks)await complete();else draw()}catch(e){fail(e)}}};draw()}
  ...
  bridge('runtime_ready',{contentId:DATA.contentId,version:${version}});
