...
export const SERVER_GAME_DELIVERY_VERSION = '4.0.0';
const casualBackgrounds = Object.freeze({
  decision: '/assets/games/server-casual-v4/security-world.png',
  memory: '/assets/games/server-casual-v4/memory-world.png',
});
export const serverCasualBackground = mode => Object.hasOwn(casualBackgrounds, mode) ? casualBackgrounds[mode] : null;

// Upgrade only same-origin asset attributes on the delivered HTML copy. Inline
// DATA, experiment code, third-party URLs and persisted artifact bytes stay intact.
export function upgradeServerGameRuntimeReferences(html) {
  if (typeof html !== 'string') return html;
  return html.replace(/<!--[\s\S]*?-->|<script\b[^>]*>[\s\S]*?<\/script\s*>|<link\b[^>]*>/gi, block => {
    if (block.startsWith('<!--')) return block;
    const opening = /^<(script|link)\b[^>]*>/i.exec(block);
    if (!opening) return block;
    const kind = opening[1].toLowerCase();
    const upgraded = opening[0].replace(/(\s)(src|href)(\s*=\s*)(?:(["'])(.*?)\4|([^\s>]+))/gi,
      (attribute, space, name, equals, quote, quotedValue, bareValue) => {
        if (name.toLowerCase() !== (kind === 'script' ? 'src' : 'href')) return attribute;
        const value = quotedValue ?? bareValue;
        const expected = kind === 'script' ? 'js' : 'css';
        if (!new RegExp(`^/server-game-runtime-v3\\.${expected}(?:\\?[^#]*)?(?:#.*)?$`).test(value)) return attribute;
        const delimiter = quote || '';
        return `${space}${name}${equals}${delimiter}/server-game-runtime-v3.${expected}?v=${SERVER_GAME_DELIVERY_VERSION}${delimiter}`;
      });
    return upgraded + block.slice(opening[0].length);
  });
}
...
export function resolveGameConfig(content, payload = {}) {
  ...
  if (supplied.length || /钱包|钓鱼|签名|私钥|助记词|phish|wallet|security/i.test(theme)) return {
    mode: 'decision', title, gameKey: 'safety-workshop', background: serverCasualBackground('decision'),
  ...
  return { mode: 'memory', title, gameKey: garden ? 'stellar-farm' : 'rune-circuit', background: serverCasualBackground('memory'),
