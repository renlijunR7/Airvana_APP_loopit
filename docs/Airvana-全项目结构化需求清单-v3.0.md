...
> 版本：v3.2
> 更新日期：2026-08-21（缺口清单全部完成 + P2 管理面已接工作台 UI）
...
| ID | 需求 | 状态 | 说明 |
| --- | --- | --- | --- |
| G-01 | 自动化测试 | **DONE** | 298/298 通过，26 个测试文件，覆盖状态机、权限边界、幂等、频率限制、工作台接线契约 |
...
| P2-3 | 受控实验与灰度 | 仅可优化字段、稳定分桶、状态机守卫、可回滚 |
...
1. **实验分支未作用于成品运行时**：`experiment_assignments` 已是服务端权威分桶，但成品渲染尚未按 `variant/control` 切换；UI 已标注为待接能力，不在界面上伪造实验效果。
2. **G-10 深化（可选）**：`public/` 127 MB 中 ai-twin 仍占 81 MB；若要 Web 端也降到 100 MB 以下，需把 HeyGen 样片移到外部存储或按需下载。
3. **E-10 律师复核**：法律文本主体信息已填，正式上线前需目标法域律师出具意见。
